...
$wilayah = mysqli_query($connection, "SELECT user.puskesmas, user.nama FROM user WHERE username = '$user'");
$wilayahPuskesmas = mysqli_fetch_array($wilayah);

$puskesmas  = $wilayahPuskesmas['puskesmas'];
$nama       = $wilayahPuskesmas['nama'];
...
$query = mysqli_query($connection, "SELECT hasil.username, hasil.goal, hasil.real1, hasil.option1, hasil.will, hasil.tgl, tema.nama_tema, kuis.nama_kuis, user.puskesmas, user.nama, user.level
                                    ...
                                    WHERE hasil.username = '$user'
                                    ORDER BY hasil.tgl ASC");
$html = '<div style="margin-bottom: 50px;"><center><h3>Hasil Ecoaching : ' . $nama . ' - ' . $puskesmas . '</h3></center><div><hr><br>';
$html .= "<strong>Tanggal Download : " . tanggal_indonesia(date('Y-m-d')) . "</strong>";
$html .= '<table border="1" width="100%" style="margin-top: 50px;" cellpadding="5" cellspacing="0">
 <tr>
 <th width="15">No</th>
 <th>Nama</th>
