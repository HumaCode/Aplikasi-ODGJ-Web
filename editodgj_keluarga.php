	...
	<div class="container" style="margin-top:60px">
		...
		<div class="panel panel-default">
				...
				<div class="form-bottom">
					<form role="form" action="" method="post" class="login-form">
						...
						<div class="form-group">
							...
							<textarea rows="3" name="status" class="form-control" required><?php echo $d['status']; ?></textarea>
						</div>
			...
			<div class="panel-footer">
				<a href="data_odjg_keluarga.php"><button type="button" class="btn btn-block btn-warning btn-lg">Kembali</button></a>
			</div>
		</div>
...
if (isset($_POST['submit'])) {
	...
	mysqli_query($connection, "UPDATE `odgj` SET `nama` = '$nama', `keluarga` = '$keluarga', `tempat_lahir` = '$tmpt', `tgl_lahir` = '$tgl', `alamat` = '$alamat', `pekerjaan` = '$pekerjaan', `pendidikan` = '$pendidikan', `awal_sakit` = '$awal', `rawat` = '$rawat', `status` = '$status', `posisi` = '$lokasi', `obat` = '$obat', `puskesmas` = '$puskesmas' WHERE `odgj`.`nik` = '$nik'");
	...
	echo "
        <script>
			Swal.fire(
				...
				window.location='data_odjg_keluarga.php';
			})
        </script>
    ";
}
